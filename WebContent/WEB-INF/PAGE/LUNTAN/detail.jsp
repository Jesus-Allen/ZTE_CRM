...
<br/>
	<div class="main w">
		...
		<div class="itemT w">
			<h1>
				<span>话题：<%=topic.getTitle()%></span>
			</h1>
		</div>
		<%if (p==1) {%>
		<div class="itemL ">
			<div class="info">
				用户名：<%=topicUser.getUserName()%><br /> <img
		...
		<div class="itemL ">
			...
			<div class="content">
				<div>
					编辑区:<a href="luntanupdate.html?uId=<%=replyUser.getUserId()%>&boardId=<%=boardId %>&topicId=<%=topicId %>&replyId=<%=reply.getReplyId()%>">修改</a>|<a href="admin/delete.html?uId=<%=replyUser.getUserId()%>&boardId=<%=boardId %>&topicId=<%=topicId %>&replyId=<%=reply.getReplyId()%>">删除</a>
				</div>
